...
const BasicBarChartOption = ({columns, options, changeOptions}) => {
    return (
        <>
            ...
            <FormControl sx={{ m: 1, maxWidth: 200 }} size="small">
                ...
                <Slider
                    size="small"
                    value={options.xPadding}
                    aria-label="Small"
                    valueLabelDisplay="auto"
                    min={0} max={1} step={0.01}
                    onChange={(e) => {changeOptions({...options, xPadding: e.target.value})}}
                />
            </FormControl>
        </>
      );
}
export default BasicBarChartOption;